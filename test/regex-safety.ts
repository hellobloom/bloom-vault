...
describe('RegEx', () => {
  it('Basic auth regular expression is safe', () => {
    assert.strictEqual(safe_regex(regularExpressions.requestUtils.basicAuth), true)
  })

  it('Auth uuid regular expression is safe', () => {
    assert.strictEqual(safe_regex(regularExpressions.auth.uuid), true)
  })
  ...
  it('Fingerprint regular expressions are safe', () => {
    ...
    for (const regExPropKey in fingerPrintRegExps) {
      ...
      assert.strictEqual(safe_regex(regex), true)
    }
  })
})
